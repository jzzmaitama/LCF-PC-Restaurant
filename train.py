...
from utils.data_utils import PCEProcessor, convert_examples_to_features
from model.lcf_pce import LCF_PCE
...
def main(config):
    ...
    if args.dataset in {'camera', 'car', 'phone', 'notebook'}:
        ...
        model = LCF_PCE(bert_base_model, args=args)
    else:
        model = LCF_PCE(bert_base_model, args=args)
    ...
    def evaluate(eval_APC=True,eval_emotion=True):
        ...
        for input_ids_spc, input_mask, segment_ids, label_ids, polarities, valid_ids, l_mask,emotions in eval_dataloader:
            ...
            with torch.no_grad():
                # ate_logits, apc_logits = 
                 apc_logits,emotion_logits = model(input_ids_spc, segment_ids, input_mask,
        ...
        return apc_result,emotion_result
    def save_model(path):
    ...
    def train():
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                loss_apc,loss_emo = model(input_ids_spc,segment_ids,input_mask, label_ids, polarities, valid_ids,
                                           l_mask,emotions)

                # Calculate the weighted loss
                loss =  loss_apc + loss_emo
                loss.backward()
                ...
                if global_step % args.eval_steps == 0:
                    if epoch >= args.num_train_epochs - 2 or args.num_train_epochs <= 2:
                        ...
                        apc_result,emotion_result = evaluate()
                        # apc_result emotion_result = evaluate()
        ...
        return [max_apc_test_acc, max_apc_test_f1,max_emotion_test_acc,max_emotion_test_f1]
...
if __name__ == "__main__":
    ...
    for config in exp_configs:
        ...
        results = []
        max_apc_test_acc, max_apc_test_f1, max_emotion_test_acc, max_emotion_test_f1 = 0, 0, 0, 0
        for i in range(n):
            ...
            apc_test_acc, apc_test_f1, emotion_test_acc, emotion_test_f1 = main(config)
            ...
            logger.info(
                'max_apc_test_acc: {}\tmax_apc_test_f1: {} \tmax_emotion_test_acc: {}\tmax_emotion_test_f1: {}'
                .format(max_apc_test_acc, max_apc_test_f1, max_emotion_test_acc, max_emotion_test_f1))
